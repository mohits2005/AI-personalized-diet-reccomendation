...
load_dotenv()
Aapi_key=os.getenv("GROQ_API_KEY")
...
prompt_template_ressto = PromptTemplate(
    input_variables=['age', 'gender', 'weight', 'veg_or_nonveg', 'disease', 'region', 'allergics', 'foodtype'],
    template=(
...
chain = LLMChain(llm=llm_resto, prompt=prompt_template_ressto)
...
breakfast_names = re.findall(r'Restaurants:\s*(.*?)\n\n', results, re.DOTALL)
...
restaurant_name = clean_list(restaurant_names[0]) if restaurant_names else []
